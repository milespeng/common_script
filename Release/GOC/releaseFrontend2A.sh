#! /bin/bash
# production flash synchronization and deploy client to gameserer script
...
comp_dir=/home/qa/ProductionPackage/build/goc_a/onlineGameFlash.war/comp_conf
salt_package_dir=/srv/salt/packages/goca
client_bak_dir=/home/qa/deployment/bak/goca/
...
backup=$s3url/miles/test_goc_a_compatibility
...
#deploy_clientServer=Prod-GOC-Game-SrvA01,Prod-GOC-Game-SrvA02
...
backend_deploy(){
   ...
   #Upload configuration files to S3
	...
	#make tar in /srv/salt/package/goca/client.tar.gz
	cd $salt_package_dir
    ...
    do    
        salt_update=`salt "$deploy_clientServer_one" state.sls sls.upload pillar='{'fileName': { client: client.tar.gz }, 'project_name': goca,'desc_dir': /srv/apache-tomcat/webapps}'`
        if [ `echo $salt_update | awk '{print $(NF-5)}'` = "0" ]
            ...
            salt_deploy=`salt "$deploy_clientServer_one" state.sls sls.deploy pillar='{'fileName': { client: client.tar.gz },'project_name': goca,'desc_dir': /srv/apache-tomcat/webapps}'`
...
backup2comp(){
    ...
    do
        ...
        sed -i "s/test_goc_a/test_goc_a_compatibility/g" $comp_dir/$file/xml/$config 
